...
Djinni code generator was extended to allow direct export of C++ modules to React Native Javascript modules.

For an example usage, see `example-react-native/ExampleProject`, in particular:
- `example-react-native/ExampleProject/App.js` - Javascript implementation.
- `example-react-native/ExampleProject/idl/main.djinni` - Djinni interface definition of the module.
- `example-react-native/ExampleProject/cpp/DemoModuleImpl.cpp` - Module implementation.

## Changes compared to original Djinni

New "React Native" extension was added (`+r`).
Interfaces marked with such extension will be exported as React Native modules.
There are certain limitations:
- Only certain primitives and wrapper classes (described below) can be used in interface definition to ensure compatibility with Javascript.
- `JavascriptPromise` type can be used only as a last parameter of a method.
- Methods cannot return value (only callbacks/promises can be used to pass data).

## Additional classes

Following classes were introduced:
- `ReactBridge` - factory class for all Javascript types. Also allows emitting global events in the Javascript layer.
- `JavascriptMap` & `JavascriptMapKeyIterator`, `JavascriptArray` - wrapper classes for the corresponding Javascript data structures. Please note that the instances passed as arguments are readonly. Writable objects can be created with the `ReactBridge` instance.
- `JavascriptObject` - dynamic wrapper for any type.
- `JavascriptCallback`, `JavascriptPromise` - used to pass data from the module to Javascript layer.
- `Job`, `JobQueue`, `JobDispatcher` - classes useful in the C++ layer to safely invoke code from native thread. JobDispatcher ensures that the given code will be executed within the context of Java VM on Android.

See `support-lib/react.djinni` and the subdirectories in `support-lib/` for implementation details.

## Running sample project

`example-react-native/ExampleProject/` is a simple "ejected" React Native project, which means you will have to open it with Android Studio and/or XCode to run it.
Additionally for Android you will have to launch React Native console by running `yarn start` in `example-react-native/ExampleProject/`.
You may also have to first run `yarn install` within that directory to install all React Native dependencies.

## Development

You can regenerate the sample project classes by running `example-react-native/ExampleProject/idl/generate_wrappers.sh`.

To generate the code for support-lib classes simply run `support-lib/generate_react.sh`.
